...
const App: React.FC = () => {
  ...
  return (
    <div className="flex h-screen w-screen bg-slate-950 text-slate-200 font-sans overflow-hidden">
      ...
      {/* Sidebar Navigation */}
      <div className="w-16 bg-slate-900 border-r border-slate-700 flex flex-col items-center py-6 gap-8 z-20">
        <div 
        ...
        <nav className="flex flex-col gap-6 w-full items-center">
          <button 
            onClick={() => setActiveTab('dashboard')}
            className={`p-3 rounded-xl transition-all duration-200 ${activeTab === 'dashboard' ? 'bg-slate-800 text-indigo-400 shadow-inner' : 'text-slate-500 hover:text-slate-200 hover:bg-slate-800/50'}`}
            title="Dashboard"
          >
          ...
          <button 
            ...
            className={`p-3 rounded-xl transition-all duration-200 ${activeTab === 'streams' ? 'bg-slate-800 text-indigo-400 shadow-inner' : 'text-slate-500 hover:text-slate-200 hover:bg-slate-800/50'}`}
            title="Live Streams"
          >
            <Radio size={20} />
          </button>
          <button 
            ...
            className={`p-3 rounded-xl transition-all duration-200 ${activeTab === 'settings' ? 'bg-slate-800 text-indigo-400 shadow-inner' : 'text-slate-500 hover:text-slate-200 hover:bg-slate-800/50'}`}
            title="Settings"
          >
            <Settings size={20} />
          </button>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Top Header */}
        <header className="h-14 border-b border-slate-700 bg-slate-900/50 backdrop-blur flex items-center justify-between px-6 flex-shrink-0">
          <div className="flex items-center gap-4">
            <h1 className="text-lg font-bold tracking-widest font-mono text-white">URBAN PULSE <span className="text-indigo-500">SENTINEL</span></h1>
            ...
            {backendError ? (
               ...
               <div className="flex items-center gap-2 text-xs font-mono text-emerald-400 bg-emerald-950/30 px-2 py-1 rounded border border-emerald-900">
        ...
        {activeTab === 'dashboard' && (
          <main className="flex-1 flex overflow-hidden animate-in fade-in duration-300">
            ...
            <div className="flex-1 flex flex-col min-w-0">
              ...
              {/* Bottom: Streams (40%) */}
              <div className="flex-[2] p-4 pt-2 border-t border-slate-800 bg-slate-900/30">
